...
def submit_sdk_logs(
    ...
    for event in body.events:
        if event.eventType == SDKLogEventType.HISTORICAL_SYNC_START:
            mark_historical_sync_started(user_id, provider, (item.type for item in event.dataTypeCounts))
    completed_types = [
        event.dataType for event in body.events if event.eventType == SDKLogEventType.HISTORICAL_TYPE_SYNC_END
    ]
    if completed_types:
