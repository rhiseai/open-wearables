...
class TestInsufficientData:
    ...
    def test_insufficient_message_is_human_readable(self):
        result = calculate_resilience_score([])
...
class TestEdgeCases:
    def test_all_identical_hrv_scores_100(self):
    ...
    def test_none_values_are_filtered_before_calculation(self):
    ...
    def test_zero_values_are_filtered_before_calculation(self):
    ...
    def test_hrv_cv_is_rounded_to_one_decimal(self):
        result = calculate_resilience_score(NORMAL_WEEK)
...
class TestScoreScaling:
    def test_elite_cv_yields_100(self):
        """CV ~5.5% is below the 7% ceiling; score must be capped at 100."""
        result = calculate_resilience_score(ELITE_WEEK)
        assert result["resilience_score"] == 100

    def test_volatile_cv_yields_0(self):
    ...
    def test_cv_exactly_at_ceiling_yields_100(self):
    ...
    def test_cv_at_floor_yields_0(self):
        """CV == cv_floor (40%) should produce exactly 0."""
        result = calculate_resilience_score(VOLATILE_WEEK)
        assert result["resilience_score"] == 0

    def test_normal_population_cv_yields_moderate_score(self):
    ...
    def test_score_decreases_monotonically_with_rising_cv(self):
        ...
        assert scores == sorted(scores, reverse=True), (
            f"Scores should decrease as variability grows, got: {scores}"
        )

    def test_config_ceiling_and_floor_are_respected(self):
...
class TestClinicalCategory:
    def test_elite_stability_category(self):
    ...
    def test_boundary_at_10_pct_is_elite(self):
    ...
    def test_boundary_above_25_pct_is_volatile(self):
        """CV > 25% must be 'Volatile'."""
        result = calculate_resilience_score(VOLATILE_WEEK)
        assert result["hrv_cv_percentage"] > 25.0
        assert result["clinical_category"] == "Volatile"

    def test_category_is_never_none_on_success(self):
...
class TestReturnShape:
    ...
    def test_insufficient_response_has_all_required_keys(self):
        result = calculate_resilience_score([])
        assert self._REQUIRED_INSUFFICIENT_KEYS.issubset(result.keys())

    def test_status_is_success_on_valid_input(self):
        assert calculate_resilience_score(ELITE_WEEK)["status"] == "success"

    def test_status_is_insufficient_data_on_short_input(self):
        assert calculate_resilience_score([50.0, 51.0])["status"] == "insufficient_data"

    def test_nights_used_equals_valid_count(self):
        result = calculate_resilience_score(ELITE_WEEK)
        assert result["nights_used"] == 7

    def test_nights_used_is_five_minimum(self):
