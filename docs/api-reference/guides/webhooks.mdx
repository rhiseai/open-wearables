...
| `sleep.updated` | An existing sleep session was revised or finalized. |
| `menstrual_cycle.created` | A new menstrual cycle record was saved. |
| `activity.created` | A new generic activity session was saved. |

### Timeseries Events

Fired per ingestion batch — one canonical group event per distinct `(user, provider, series_type)` combination in a sync run. Open Wearables coalesces writes from the same Mobile SDK batch instead of emitting both group and granular duplicates. Each event carries the `samples` array, so consumers can store data directly from the webhook without issuing follow-up API calls.
...
<Note>
  **Large batches** are split by the exact UTF-8 JSON payload size, not by an estimated sample count. Each delivery stays at or below 900 KiB, leaving headroom under Svix's 1 MB limit. Each chunk includes `chunk_index` (0-based) and `total_chunks`; `sample_count` always reflects the total across all chunks.
</Note>

<Note>
  **Mobile SDK syncs are bounded.** One realtime SDK upload emits at most 64 coalesced product events. Initial historical exports suppress per-record realtime notifications and emit the compact `sync.completed` lifecycle event instead. Use the pull API for historical backfill and reconciliation.
</Note>
