...
| Type | Unit | Description |
|------|------|-------------|
| `dietary_energy_consumed` | kcal | Dietary energy consumed |
| `dietary_protein` | g | Protein consumed |
| `dietary_carbohydrates` | g | Carbohydrates consumed |
| `dietary_fat_total` | g | Total fat consumed |
| `dietary_fiber` | g | Fiber consumed |
...
| Type | Unit | Description |
|------|------|-------------|
| `menstrual_flow` | score | Menstrual flow intensity (0=none … 4=heavy) |
| `cervical_mucus_quality` | score | Cervical mucus quality (HealthKit enum) |
| `ovulation_test_result` | score | Ovulation test result (HealthKit enum) |
| `basal_body_temperature` | °C | Basal body temperature (separate from `body_temperature`) |

### Mindfulness

| Type | Unit | Description |
|------|------|-------------|
| `mindful_minutes` | minutes | Mindful session duration |
...
| `hydration` | mL | Hydration / dietary water |
...
The `resolution` parameter downsamples timeseries server-side:
...
Aggregation returns **one sample per (bucket, series type, data source)**:

- **How values combine** follows the series type: cumulative types are summed
  (`steps`, `energy`, `distance_walking_running`, `mindful_minutes`), rate and
  level types are averaged (`heart_rate`, `heart_rate_variability_rmssd`,
  `oxygen_saturation`, `weight`), and peak types take the maximum.
- **Data sources stay separate.** A watch and a phone reporting the same minute
  come back as two samples, each with its own `source`, so you can apply your
  own priority rules.
- **Daily totals stay separate** from intraday samples (`is_daily_total`), so a
  provider that sends both is never double-counted inside a bucket.
- **Buckets are aligned to the wall clock**, not to `start_time`: at `1hour`,
  a bucket always starts on the hour. The `timestamp` is the bucket start (UTC),
  and `pagination.total_count` counts buckets, not raw samples.
  ...
  "data": [
    {
      ...
      "value": 63.5,
      "unit": "bpm",
      "source": { "provider": "apple", "device": "Watch7,1" }
    }
  ],
  "pagination": { "has_more": true, "next_cursor": "...", "total_count": 24 },
  "metadata": { "resolution": "1hour", "sample_count": 1 }
}
```
